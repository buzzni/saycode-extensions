...
test('/clear and /reset map to the clear operation Core classifies', async () => {
  ...
  assert.deepEqual(parseTelegramCommand('/clear'), { operation: 'clear' })
  // Unlisted alias, same as Core's catalogue.
  assert.deepEqual(parseTelegramCommand('/reset'), { operation: 'clear' })
  ...
  assert.deepEqual(parseTelegramCommand('/clear now'), { operation: 'clear' })
  assert.deepEqual(parseTelegramCommand('/CLEAR'), { operation: 'clear' })
  // Still plain text when it is not the command word.
  assert.deepEqual(parseTelegramCommand('clear the cache please'), { operation: 'prompt', text: 'clear the cache please' })
})
